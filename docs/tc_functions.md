...
A key objective of the Primary Industry is to generate materials that maximise the potential for component production. This is achieved by applying knowledge obtained from science and engineering about the physical world. Primary production keeps executing a material supply loop until it produces a quality Material Interface (MI). That means while the following conditions apply:
...
We of course have access to this information, because we are the users. Users project demand, and these are modelled in the TC-Node Core in terms of Organizations.


## Organizations

In the originating [sharpNode](tc_history.md#sharpnode) schema design of 2002, organizations and their people are expressed as components in a projected namespace. This has been removed in the TC-Node Core because it confused the users. Perhaps there may be a way to re-introduce it without alarming them, but it is instructive to explore the original approach.    

### Namespaces

An organization is just that - it is a subject that has organs (components) that are connected in an ordered way, just like an object's Workflow. And like the Workflow it excretes planes of abstraction. This can be formalised with Namespaces, which are like an address, except they explicitly express the planes of abstraction that identify the subject, in the order of supply. 
...
Here is the organizational structure of the Bell Maker, deduced from its namespace collection:

![Namespace Structure](../img/functions_organization.jpg)

It looks like the [object structure](#object-structure) of a Workflow. That is because it occupies the same reality, but it is in fact very different. There is no Consumer or Production Network flowing through these abstract planes, despite being produced by and processing them. Instead, the organization as subject brings its structure into being by projecting itself into the technological world.
...
Such technology, like life, cannot be a simulation; because it is a force that is projecting from outside its abstract plane. You should therefore understand that every living thing can also pass this alternative to the Turing Test, which is not the case for the original, or indeed any technology produced to date. Can future technology ever pass this test? Never say never. But whilst today, Turing’s ship is in the offing, this new ship has sailed over the horizon and disappeared.
