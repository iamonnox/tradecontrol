...
From these statements we add three elements to the balance sheet: 
...
In the good old days, companies had two bank accounts: a current account for trading and a reserve account for savings and tax. Because the banks had access to your reserves, they could lend it out and in return they paid you interest. In this way, banks competed in the interest rates they provided. Today, the banks still have access to your money, but pay you virtually nothing in return (<1%). Therefore, the motivation for maintaining a reserve account is mainly operational. It removes savings and tax obligations from the Company Statement, leading to a clearer projected balance. So, although the design is from bygone era, it is still useful. Which is more than could be said for the banks.

You can use the [Trade Control HD Wallet](tc_bitcoin.md) to replace the banks, with more advanced features, such as [namespacing](tc_functions.md#namespaces). Although there is no technological obstacle standing in your way, for political reasons, it is probably not a practical solution at this time. It will be waiting for you when and if that should change. Your balance sheet will then be automatically generated for Companies House in bitcoin. You can do that now, but unfortunately, they will not accept it.
...
To produce the full balance sheet, we union these three views and join the result to the [financial periods](https://github.com/tradecontrol/tc-nodecore/blob/master/src/tcNodeDb/Cash/Views/vwBalanceSheetPeriods.sql).  The result is ordered by polarity, liquidity, asset and period before applying the [asset charge](#asset-charge) algorithm.
