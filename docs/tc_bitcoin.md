...
To receive her first Bitcoin, Alice creates a new wallet to which she alone has the private keys. If only all her financial transactions could be from this wallet! She would no longer be reliant on the money-printing fiat currency of the banks, with Caesar’s head upon its coins. She would be a sovereign individual ^2^. But is that true? Of all the transactions involved in trade, the financial transaction is the most trivial. In the accompanying paper, [Trade Control Functions](tc_functions.md), I explain how trade works and how it maps onto the schema design. Sovereignty is a concept, but like all reality, it is underpinned by functions. Therefore, I can apply my theory of technological production to Alice’s Bitcoin purchase but on the trading side of the transaction.

Physical trade is a collaborative venture dependent upon [production](tc_functions.md#production) and [organisation](tc_functions.md#organisations), for without production there is nothing to trade and without organisation there are no [interfaces](tc_functions.md#interfaces) to produce. This fundamental fact is encapsulated in my life-affirming [alternative to the Turing Test](tc_functions.md#turing-test-v2). The transactions that take place in production do so across spatial and temporal [workflows](tc_functions.md#workflows). These naturally organise themselves into [Consumer](tc_functions.md#consumer-networks) and [Production Networks](tc_functions.md#production-networks). They do so because there are [rules of abstraction](tc_functions.md#component-definition) that determine the production and consumption of a [subject’s projected interfaces](tc_functions.md#subjects-and-objects). It is why the Trade Control node is recursive internally ([BOM Demonstration](https://github.com/TradeControl/tc-office/blob/master/docs/tc_demo_manufacturing.md)) as well as externally between nodes ([Network Demonstration](https://github.com/tradecontrol/tc-network/blob/master/docs/tc_network_demo.md)).

Bob’s Café serves Alice her beverage. There is then a Consumer Network that flows through the café: various kinds of beans shipped from coffee-growing regions around the globe, cow’s milk from the Dairy Industry, oat milk from Arable farming, sugar, and every other offering it sells, like the ingredients for its cakes or disposable straws. All the stuff that is consumed in the production of its interfaces, including utilities like water supply and electricity, or waste products like sewerage. But there must also be a Production Network to enable the café to deliver its product: the industrial coffee machines, the utensils, the ovens, fridges and sinks, the building itself and the infrastructure that supports it, as well as labour. Stuff not consumed in the act of production: tools that just wear out, by entropy or design; staff that come and go. Then there is the User Interface: the taste of the coffee, the snacks, the crockery, the furniture and décor, the quality of service, the location and clientele, the cost.
...
Now Bitcoin has been successfully mapped onto the Cash Account, as UOA, ask yourself the following:

> Who owns this wallet and under what conditions?

Here are two pointers:

1.  The Bitcoin community falsely presents a Bitcoin transaction as a
    double-entry book-keeping ledger entry:
...
2.  The most sovereign individuals in the world live in [the City of Gold](https://en.wikipedia.org/wiki/Monaco).
