		...
		SELECT 
			...
			(SELECT SUM( Org.tbAccount.CurrentBalance)
				FROM         Org.tbAccount LEFT OUTER JOIN
									  Cash.tbCode ON Org.tbAccount.CashCode = Cash.tbCode.CashCode
				WHERE     ( Org.tbAccount.AccountClosed = 0) AND ( Cash.tbCode.CashCode IS NULL)) AS Balance
	)
...
DROP FUNCTION IF EXISTS Cash.fnStatementReserves;
DROP FUNCTION IF EXISTS Cash.fnTaxCorpStatement;
DROP FUNCTION IF EXISTS Cash.fnCorpTaxCashCodes
DROP VIEW IF EXISTS Cash.vwCorpTaxConfirmedBase
DROP VIEW IF EXISTS Cash.vwCorpTaxConfirmed
DROP VIEW IF EXISTS Cash.vwCorpTaxTasksBase
DROP VIEW IF EXISTS Cash.vwCorpTaxTasks
DROP FUNCTION IF EXISTS Cash.fnTaxCorpOrderTotals
DROP VIEW IF EXISTS Cash.vwStatementCorpTaxDueDate
DROP VIEW IF EXISTS Task.vwVatFull
DROP VIEW IF EXISTS Task.vwVatConfirmed
DROP FUNCTION IF EXISTS Cash.fnTaxVatOrderTotals
DROP VIEW IF EXISTS Cash.vwStatementVatDueDate
DROP FUNCTION IF EXISTS Cash.fnStatementTaxEntries;
DROP VIEW IF EXISTS Cash.vwStatementTasksConfirmed
DROP VIEW IF EXISTS Cash.vwStatementTasksFull
DROP FUNCTION IF EXISTS Cash.fnStatementCompany
go
