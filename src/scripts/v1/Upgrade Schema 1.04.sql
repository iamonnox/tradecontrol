...
ALTER TABLE tbActivity WITH NOCHECK ADD
	[RegisterName] [nvarchar] (50) NULL	
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[FK_tbActivity_tbSystemRegister]') and OBJECTPROPERTY(id, N'IsForeignKey') = 1)
ALTER TABLE [dbo].[tbActivity] DROP CONSTRAINT FK_tbActivity_tbSystemRegister
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[tbSystemRegister]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[tbSystemRegister]
GO
...
ALTER TABLE [dbo].[tbSystemRegister] WITH NOCHECK ADD 
...
ALTER TABLE [dbo].[tbSystemRegister] ADD 
	CONSTRAINT [DF_tbSystemRegister_NextNumber] DEFAULT (1) FOR [NextNumber]
GO

ALTER TABLE [dbo].[tbActivity] ADD 
	CONSTRAINT [FK_tbActivity_tbSystemRegister] FOREIGN KEY 
	(
		[RegisterName]
	) REFERENCES [dbo].[tbSystemRegister] (
		[RegisterName]
	) ON UPDATE CASCADE
GO

if not exists(select ItemName from tbProfileObjectDetail where ItemName = 'RegisterName')
	begin
...
ALTER  PROCEDURE dbo.spTaskNextCode
	(
